        ...
        for j in range(self.hps['num_iter']):
            try:
                ...
                for j in range(0, self.num_online, self.sub_batch_size):
                    ...
                    # print('finetune_new.py gfn_batch.flat_rewards ', type(gfn_batch.flat_rewards), gfn_batch.flat_rewards.shape)
                    ...
                    gfn_batch.avg_qed = np.average(rew_tup[1][-1][0])
                    gfn_batch.avg_tpsa = np.average(rew_tup[1][0][0])
                    # gfn_batch.avg_num_rings =  np.average(rew_tup[1][1][0])
                    gfn_batch.avg_sas =  np.average(rew_tup[1][1][0])
                    gfn_batch.avg_zinc_rad = np.average(rew_tup[3])

                    gfn_batch.online_flat_task_rewards = torch.Tensor(flat_task_reward_online)
                    gfn_batch.avg_dock_score_on = np.average(docking_scores_on)
                    gfn_batch.sim_on = np.average(sim_on)

                    # print('finetune_new.py gfnbatch mol wt ', gfn_batch.avg_mol_wt)
                    # print('finetune_new.py gfnbatch qed ', gfn_batch.avg_qed)
                    # print('finetune_new.py gfnbatch sas ', gfn_batch.avg_sas)
                    # print('finetune_new.py gfnbatch logP ', gfn_batch.avg_logP)
                    # print('finetune_new.py gfnbatch zinc_Rad ', gfn_batch.avg_zinc_rad)

                    # avg_batch_fwd_traj_len, _, _ = Utils.avg_traj_len([None], online_trajs )

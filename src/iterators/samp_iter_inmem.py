...
class Sampling_Iterator_InMemory(IterableDataset):
    def __init__(self, datafile, pretrainer, wrapped_model, dev, mixer) -> None:
        ...
        self.legal_atoms = set(pretrainer.hps['atoms'])
        self.allow_charge = True if len(pretrainer.ctx.charges)>1 else False
        self.df = pd.read_csv(self.hps['data_root']+datafile)
    ...
    def __iter__(self):
        ...
        for i in range(self.hps['num_iter']):
            ...
            while self.smiles_idx < len(worker_smiles):
                try:
                    smiles_batch = worker_smiles[self.smiles_idx:self.smiles_idx+self.batch_size//2]  
                    ...
                    for j in range(0,max(len(online_trajs),len(offline_trajs)),self.sub_batch_size):
                        ...
                        gfn_batch = self.algo.construct_batch(sub_online_trajs+sub_offline_trajs, 
...
def build_train_loader(hps, pretrainer, mixer, datafile=None):
    ...
    iterator = Sampling_Iterator_InMemory(datafile,pretrainer, wrapped_model,dev, mixer)

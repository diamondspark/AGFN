...
class TrajectoryBalance:
    ...
    def construct_batch(self, trajs, cond_info, log_rewards):
        ...
        """
        # print('traj_bal.py cond_info ', cond_info.shape)
        torch_graphs = [self.ctx.graph_to_Data(i[0], cond_info[tidx][None, :]) 
                        for tidx, tj in enumerate(trajs) for i in tj["traj"]]
        actions = [
            self.ctx.GraphAction_to_aidx(g, a) for g, a in zip(torch_graphs, [i[1] for tj in trajs for i in tj["traj"]])
        ]
        batch = self.ctx.collate(torch_graphs)
        batch.traj_cond_info = cond_info
        batch.traj_lens = torch.tensor([len(i["traj"]) for i in trajs])
        batch.log_p_B = torch.cat([i["bck_logprobs"] for i in trajs], 0)
        # print('trajbal.py construct batch len-trajs ', len(trajs))
    ...
    def compute_batch_losses(self, model: TrajectoryBalanceModel, batch: gd.Batch, num_bootstrap: int = 0):
        ...
        # This feels wrong... 
        # print('traj_bal.py ',batch.cond_info.shape, batch.traj_lens.shape)
        traj_cond_info = batch.cond_info
        # print('tb.py batch.cond_info ', batch.cond_info.shape, 'traj lens ', batch.traj_lens)
        # batch.cond_info = batch.cond_info.repeat_interleave(batch.traj_lens, 0)
        ...
        if self.p_b_is_parameterized:
            fwd_cat, bck_cat, per_graph_out, logZ_forward = model(batch)
        else:
            fwd_cat, per_graph_out, logZ_forward = model(batch)
        ...
        # log_Z = model.logZ(traj_cond_info)[:, 0]
        log_Z = model.logZ(batch.traj_cond_info)[:, 0]

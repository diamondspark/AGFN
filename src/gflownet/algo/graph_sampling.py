...
class GraphSampler:
    ...
    def sample_from_model(
        self, model: nn.Module, n: int, cond_info: Tensor, dev: torch.device, 
        random_stop_action_prob: float = 0.0, random_action_prob: float = 0.0, seed_graph =None,
    ):
        ...
        seed_graph: graph_building.env.Graph
            When doing lead optimization, use seed graph instead of env.new() to initialize trajs.
        ...
        if seed_graph is None:
            ...
            graphs = [seed_graph for i in range(n)]
        # print('graph_sampling.py seed graph ', graphs[0])
        done = [False] * n
        ...
        for t in range(self.max_len):
            if isinstance(self.ctx, FragMolBuildingEnvContext):
                torch_graphs = [
                    self.ctx.graph_to_Data(graphs[i], cond_info=cond_info[i][None, :].cpu()) for i in not_done(range(n))
                ]
            elif isinstance(self.ctx, MolBuildingEnvContext):
                torch_graphs = [
                    self.ctx.graph_to_Data(graphs[i], cond_info=cond_info[i][None, :].cpu()) for i in not_done(range(n))
                ]

            # print(f'total graphs to sample {n} not done torch graphs len {len(not_done(range(n)))}')
            ...
            # print('graph_samp.py torch graphs collate ',self.ctx.collate(torch_graphs) )
            with torch.no_grad():
                fwd_cat, *_, log_reward_preds = model(self.ctx.collate(torch_graphs).to(dev))
            ...
            if random_action_prob > 0:
                ...
                # print('graphsampling.py before randomized logits ',tmp, 'random_actions id ', is_random_action, 'after randomization logits ',fwd_cat.logits)
            ...
            # print('graphsamp random_stop actions prob ', random_stop_action_prob)
            if isinstance(self.ctx, MolBuildingEnvContext):
                if (t>0) and (random_stop_action_prob > 0): # t>0 to avoid first action being a STOP action
                    is_random_stop_action = torch.tensor(self.rng.uniform(size=len(torch_graphs)) < random_stop_action_prob).float().numpy()

                    # Create a mask for indices where is_random_action is 1
                    mask = np.where(is_random_stop_action == 1, True, False)
                    # print(f'graphsampling.py stopping {sum(mask)} out of {len(mask)} trajectories')

                    # Update actions using the mask
                    actions = [(0, 0, 0) if m else a for m, a in zip(mask, actions)]
                # print('graphsampling.py sampled STOP actions ', actions               
            ...
            for i, j in zip(not_done(range(n)), range(n)):
                ...
                if graph_actions[j].action is GraphActionType.Stop:
                    ...
                    try:
                        ...
                        # print('graph_samp.py graph_actions[j] ',graph_actions[j],' type ', type(graph_actions[j]))
